...
export const sampleManuscript: ManuscriptDraft = {
  ...
  metadata: {
    ...
    abstract:
      '本文提出一种纯前端期刊排版流程，将论文元信息结构化建模，并与 Markdown 正文渲染深度耦合，实现以最终导出排版为准的实时预览与 PDF 导出。系统在浏览器内依次完成 markdown-it 解析、学术模板渲染、分页排版、页眉页脚注入与浏览器打印，无需后端服务。实验结果表明，在常见投稿场景下，该方法可稳定生成符合期刊格式要求的高质量文档，并显著降低作者排版负担。',
...
2. 通过 markdown-it 解析、学术模板渲染与 paged.js 分页生成排版预览。
3. 预览直接复用导出分页结果，按最终 PDF 版式调整页边距与行距。
4. 通过浏览器打印导出 PDF。

> 该流程避免了“编辑器样式”和“导出样式”分离导致的版式漂移问题，并让预览页数与导出页数保持一致。
...
| Export Engine | 学术模板 + 分页页面流 | 浏览器打印 PDF |

## 3 讨论

单栏模式下，正文围绕结构块完整性进行分页控制。为了提升可读性，我们对标题、代码块、表格设置了“避免页内断裂”策略，并确保图片题注与块级公式编号在跨页后保持连续。
