...
export const exportRegressionFixture: ExportRegressionFixture = {
  metadata: {
    ...
    abstract:
      '本文构建一个纯前端导出回归样例，用于系统验证学术排版中的 WYSIWYG 一致性、分页稳定性与版式严肃性。样例覆盖长段落、分节标题、表格、图像、块级公式、列表、引用，以及通讯作者标记与联系方式行，重点观测空白首页、标题孤行、段落截断、公式残缺等问题。通过统一渲染根与分页规则约束，导出结果在预览与 PDF 之间保持高一致性，并显著降低页底硬切现象。',
...
此外，作者前言区域也纳入回归核查：通讯作者应在作者行中显示星号上标（如 \`赵婷¹,²*\`），并在作者单位下方显示 \`* 通讯作者: 赵婷 (ting.zhao@example.edu)\`。
...
![导出回归图示](/fixture-paper-figure.svg "导出回归测试图")
...
本节使用连续长段落验证分页质量。首先，若段落行距偏大且没有孤行寡行约束，页面尾部容易出现单行悬挂。其次，单栏模式下若图片、表格、公式等结构块缺少统一的避断策略，不同浏览器也可能出现落点差异。再次，若对所有段落统一设置“禁止断页”，会导致版心利用率显著下降，反而使跨页跳跃更加频繁。
...
3. 检查图表与公式是否被粗暴切断。
4. 检查预览页序与导出页序是否一致。
5. 检查通讯作者标记与说明行在预览和导出中是否一致。

> 建议在每次导出前固定纸张尺寸、页边距与行距，再进行最终版式核对，以避免参数漂移带来的页码变化。

## 4 结论 / Conclusion

本回归样例可一键注入编辑器，用于复现和验证导出一致性问题。建议在每次修改导出逻辑或分页样式后，都执行同一组手动核查步骤：逐页比对预览并执行 canvas 导出，重点检查空白首页、页序偏移、标题孤行与内容截断。`,
};
